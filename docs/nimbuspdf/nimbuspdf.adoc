...
* *18* El método _params2fact_ procesa los parámetros recibidos en la URL y los asigna a los campos de @fact si coinciden los nombres, además si se recibe el parámetro especial _go_, se llamará directamente a _after_save_. Sería una forma de emular las características del _gi_.
...
+
Tanto NimbusPDF.new en su primer argumento, como NimbusPDF.generate en su segundo, admiten un hash con las opciones _info_ y _compress_ (como está usado en el ejemplo).
