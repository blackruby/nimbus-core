...
class P2pController < ApplicationController
  ...
  def p2p_select
    mensaje tit: 'Control del proceso', msg: '¿Desea detener el proceso seleccionado?', width: 430, bot: [{label: 'Detener', busy: true, accion: :p2p_kill15}, {label: 'Forzar Detención',busy: true,  accion: :p2p_kill9}, {label: 'Cancelar'}]
    @g[:id] = params[:id]
  end
