...
class UsuariosMod < Usuario
  ...
  @campos = {
    ...
    api: {tab: 'general', gcols: 2, visible: api, grid: {hidden: !api}},
    menumin: {tab: 'general', label: 'Tablet', gcols: 2, type: :boolean, pref: true, title: 'menumin_tit'},
    audit: {tab: 'general', label: 'Auditar', gcols: 2, visible: Nimbus::Config[:audit], grid: {}},
    timeout: {tab: 'general', gcols: 2},
...
class UsuariosController < ApplicationController
  ...
  def before_save
    if @fact.password and @fact.password != ''
      ...
      # dejar la fecha de modificación del password en nil si el usuario no es API, es nuevo o le cambia la contraseña otro usuario (admin)
      # Para tener un criterio para forzar el cambio de password en el siguiente login del usuario.
      @fact.password_fec_mod = (@fact.api || @fact.id && @fact.id == @usu.id) ? ahora : nil
